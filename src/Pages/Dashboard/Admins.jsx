          ...
          <>
              ...
              <table className={`w-full table-auto ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
                ...
                <tbody>
                  {adminList && adminList.length > 0 ? (
                    adminList.map(admin => (
                      <tr key={admin.id} className={`border-b ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'} last:border-b-0`}>
                        ...
                        <td className="py-4 px-4">
                          {(() => {
                            const roleText = admin.role === 0 ? 'Admin Régulier' : 'Super Admin';
                            const roleClass = admin.role === 0 ? 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300';
                            return (
            ...
            <div className={`mt-4 flex flex-col sm:flex-row justify-between items-center text-sm p-4 rounded-md shadow ${theme === 'dark' ? 'bg-gray-700 text-gray-200' : 'bg-gray-100 text-gray-700'}`}>
              <div className="mb-2 sm:mb-0">
                Affichage de {Math.min(totalRows, (currentPage - 1) * perPage + 1)} à {Math.min(totalRows, currentPage * perPage)} sur {totalRows} administrateurs.
              </div>
