...
export function AuthContextProvider({ children }) {
    ...
    // CORRIGÉ : Ajout du /v1/ dans l'URL
    ...
    // CORRIGÉ : Ajout du /v1/ dans l'URL et ajustement de l'ordre des paramètres pour matcher le Swagger
    const resetPassword = useCallback(async (email, token, newPassword) => {
        setLoading(true);
        try {
            const response = await api.post('/api/v1/users/reset-password', { email, token, newPassword });
    ...
    const externalLoginGoogle = useCallback(async () => {
        setLoading(true);
        try {
            const response = await api.get('/api/v1/users/external-login/google');
