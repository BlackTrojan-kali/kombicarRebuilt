  ...
  return (
    ...
    <Link to={`/trip-detail/${trip.id}`} className="block">
      <div className={`card ${cardBg} w-full h-auto overflow-hidden rounded-[15px] shadow-lg dark:text-gray-200`}>
        ...
        <img src={'/default/city-1.jpg'} className='w-full h-[200px] object-cover rounded-t-[15px]' alt={`Illustration du trajet depuis ${trip.startAreaPointCreateDto?.homeTownName} vers ${trip.arivalAreaPointCreateDto?.homeTownName}`} />
        <div className='p-6 w-full relative'>
          <div className='flex justify-between items-center text-xl font-bold mb-4'>
            {/* Utilisation des chemins d'accès corrects depuis l'objet du contexte */}
            <p className={textColorPrimary}>{trip.startAreaPointCreateDto?.homeTownName || 'Départ'}</p>
            <FontAwesomeIcon icon={faArrowRight} className={textColorSecondary} />
            {/* Utilisation des chemins d'accès corrects depuis l'objet du contexte */}
            <p className={textColorPrimary}>{trip.arivalAreaPointCreateDto?.homeTownName || 'Arrivée'}</p>
          </div>
          <div>
            <span className={`font-bold text-sm ${textColorSecondary}`}>Prix à partir de</span>
            <p className='text-2xl font-bold text-green-600 dark:text-green-400'>{trip.pricePerPlace} XAF</p>
          </div>
          ...
          <div className='pt-2 pb-4 pr-10'>
            <div className='grid grid-cols-2 gap-4 text-center'>
              <div>
                ...
                <p className={`font-bold ${textColorPrimary}`}>{formatTime(trip.departureDate)}</p>
              </div>
              <div>
                <h4 className={`text-xs ${textColorSecondary}`}>Places restantes</h4>
                <p className={`font-bold ${textColorPrimary}`}>{trip.placesLeft}</p>
              </div>
            </div>
          </div>
